...
```{r genome-files}
genomes <- lapply(names(genome.files), function(g) {
    x <- Biostrings::readDNAStringSet(genome.files[[g]])
    seqinfo(x) <- GenomeInfoDb::Seqinfo(names(x), width(x), genome=g)
    x
})
```
...
longer. The option `metadata=TRUE` adds any metadata as additional
columns to the resulting data frame from the `as.data.frame` call.

```{r plot-rip, message=FALSE, warning=FALSE}
ggplot(as.data.frame(rip.ls, metadata=TRUE), aes(y=rip.composite, x=query.width)) +
    geom_point(size=1) + ylim(-3, 3) + facet_wrap(. ~ genome)
```
